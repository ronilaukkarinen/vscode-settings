...
exports.SPOTIFY_ROOT_API = "https://api.spotify.com";
...
class MusicClient {
    ...
    getHighestFrequencySpotifyGenre(genreList) {
            ...
            genreList.forEach((genre) => {
                ...
                if (genre) {
                    ...
                    tokens.forEach((token) => {
                        token = token ? token.trim() : "";
                        if (token) {
                            const tokenRegex = new RegExp("\\b" + token + "\\b", "ig");
        ...
        if (Object.keys(map).length) {
            ...
            // remove the ones that are not original
            genreList = genreList.filter((a) => a.original === true);
            // sort by rank
            genreList = genreList.sort((a, b) => b.rank - a.rank);
            // stop if the rank starts to descend and we haven't found anything
            let initialCount = genreList[0].rank;
            for (let i = 0; i < genreList.length; i++) {
                const obj = genreList[i];
    ...
    async getGenreFromSpotify(artist, artistId = "") {
        ...
        return spotifyClient
            ...
            if (resp && resp.data) {
                if (artistId) {
                    ...
                    // {genres: [...], ...}
                    genre = this.getHighestFrequencySpotifyGenre(resp.data.genres);
                }
                else {
                    if (resp.data &&
                        resp.data.artists &&
    ...
    async fetchItunesGenre(artist, song) {
        let terms = "";
        if (song && artist) {
    ...
    async refreshSpotifyToken(optionalRefreshToken = "") {
        ...
        spotifyAccountClient.defaults.headers.common["Authorization"] = `Basic ${encodedAuthPayload}`;
        ...
        let response = await spotifyAccountClient
            .post(`/api/token?grant_type=refresh_token&refresh_token=${refreshToken}`, null)
            .then((resp) => {
    ...
    async spotifyApiGet(api, qsOptions = {}, optionalAccessToken = "") {
        if (!musicStore.spotifyAccessToken) {
            return this.throwNoSpotifyTokenInfoError();
        }
    ...
    async spotifyApiPut(api, qsOptions = {}, payload = {}) {
        if (!musicStore.spotifyAccessToken) {
            return this.throwNoSpotifyTokenInfoError();
        }
        api = this.addQueryStringToApi(api, qsOptions);
    ...
    async spotifyApiPost(api, qsOptions = {}, payload = {}) {
        if (!musicStore.spotifyAccessToken) {
            return this.throwNoSpotifyTokenInfoError();
        }
        api = this.addQueryStringToApi(api, qsOptions);
    ...
    async spotifyApiDelete(api, qsOptions = {}, payload = {}) {
        if (!musicStore.spotifyAccessToken) {
            return this.throwNoSpotifyTokenInfoError();
        }
        api = this.addQueryStringToApi(api, qsOptions);
    ...
    buildSuccessResponse(resp) {
        // it's 200 by default
        const codyResp = new models_1.CodyResponse();
        if (resp.status) {
